        ...
        <template v-else>
          <div class="card-con">
            ...
            <div class="card-text-con">
              <div class="card-text">
                <h3 class="card-title">{{ post.title }}</h3>
                <h6 v-if="post.createdAt" class="leading-tight font-normal text-sm">{{ formatDate(post.date) }}</h6>
                <p class="pb-6 leading-tight">{{ post.description }}</p>
                <nuxt-link :to="`${postType}/${post.slug}`" class="post-btn" :title="`Read more about ${post.title}`"
                  >Read the full blog here!</nuxt-link
                >
              </div>
            </div>
          </div>
